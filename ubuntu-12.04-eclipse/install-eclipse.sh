...
function _download_extract(){
    ...
    case $_ext in
        tar.gz)
                ...
                curl -s $_dnld_url | tar -xvz -C "$_tmpdir" > /dev/null 2>&1
                ;;
...
function install_application(){
    ...
    _download_extract "$_download_url" "$_install_dir" eclipse
}
...
function get_download_url(){
    ...
    local _tmp_url_a=$(curl -s $_ECLIPSE_DOWNLOAD_URL | grep -A1 "Latest Release" | grep drop | awk -F\" '{print $2}')
    local _dnld_url=
    
    case "$_os" in
        MAC) 
            local _tmp_url_b=$(curl -s $_ECLIPSE_DOWNLOAD_URL$_tmp_url_a | grep eclipse-platform | grep `_lowercase $_os` | grep $_arch | awk -F"href=\"download.php" '{print $2}' | awk -F\" '{print $1}' | awk -F= '{print $2}')
            ...
            ;;
        LINUX*)
            local _tmp_url_b=$(curl -s $_ECLIPSE_DOWNLOAD_URL$_tmp_url_a | grep eclipse-platform | grep linux | grep $_arch | awk -F"href=\"download.php" '{print $2}' | awk -F\" '{print $1}' | awk -F= '{print $2}')
...
function optimize_install(){
    ...
    case "$__OS" in
        MAC)    _eclipse_ini="$_install_dir/Eclipse.app/Contents/MacOS/eclipse.ini" ;;
...
function install_plugin(){
    ...
    local _success=$("$_install_dir"/eclipse -nosplash -application org.eclipse.equinox.p2.director -repository "$_repo" -installIU "$_plugin" 2>>$__LOGFILE | grep -Poc '^Operation completed in')
